...
function TeamHealthCard({ team, orgId }: { team: TeamHealth; orgId: string }) {
  ...
  return (
    <div className={`${classificationStyle.bg} border ${classificationStyle.border} rounded-xl p-4 transition-all`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <h5 className="text-sm font-bold text-gray-800 dark:text-gray-100 truncate">{team.name}</h5>
          ...
          {team.narrative && (
            <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed mb-2">{team.narrative}</p>
          )}

          {/* Signal strip */}
          {team.signals && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-500 dark:text-gray-400 mb-2">
              <span>Progress: <span className="font-bold text-gray-700 dark:text-gray-200">{team.signals.overall_progress_pct}%</span> (exp {team.signals.expected_progress_pct}%)</span>
              <span>Velocity 7d: <span className="font-bold text-gray-700 dark:text-gray-200">{team.signals.velocity_7d}</span></span>
              <span>Overdue: <span className={`font-bold ${team.signals.overdue_count > 0 ? 'text-red-600' : 'text-gray-700 dark:text-gray-200'}`}>{team.signals.overdue_count}</span></span>
              <span>Active: <span className="font-bold text-gray-700 dark:text-gray-200">{team.signals.active_members_7d}/{team.signals.member_count}</span></span>
              <span>Discussions: <span className="font-bold text-gray-700 dark:text-gray-200">{team.signals.total_discussions}</span></span>
            </div>
          )}
      ...
      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700/50 flex items-center justify-between gap-3">
        <button
          onClick={() => setExpanded(!expanded)}
          className="text-[11px] text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 font-medium flex items-center gap-1"
        >
      ...
      {expanded && (
        <div className="mt-3 bg-white/80 rounded-lg p-3 border border-gray-200 dark:border-slate-700">
          <div className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Score Breakdown</div>
            ...
            {Object.entries(team.breakdown).map(([key, val]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  <div className="text-[11px] text-gray-600 dark:text-gray-300 w-44 flex items-center justify-between">
                  ...
                  <div className="text-[11px] font-bold text-gray-700 dark:text-gray-200 w-10 text-right">{pct}</div>
                </div>
              )
            })}
          </div>
          {team.suggested_actions.length > 0 && (
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-700">
              <div className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Suggested actions</div>
    ...
    return (
      <AppLayout user={user} organizations={organizations} selectedOrg={selectedOrg} onOrgChange={setSelectedOrg}>
        <div className="max-w-3xl mx-auto px-4 py-16 text-center">
          <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl p-12 border border-white/40 dark:border-slate-700/40 shadow-lg">
            <BarChart3 className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">Analytics Not Available</h2>
            <p className="text-gray-500 dark:text-gray-400">Analytics and AI Insights are available for teachers, adviser, and admins only.</p>
          </div>
        </div>
      </AppLayout>
    )
        ...
        {overviewLoading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl p-5 border border-white/40 dark:border-slate-700/40 shadow-lg hover:shadow-xl transition-shadow">
              ...
              <div className="p-6">
                {!insights && !insightsLoading && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-1">Click &quot;Generate Insights&quot; to get AI-powered analysis</p>
                    ...
                    {insights.keyInsight && (
                      <div className="flex items-start gap-3 bg-amber-50 rounded-xl p-4 border border-amber-200">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-700 dark:text-gray-200">{insights.keyInsight}</p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="bg-white/60 rounded-xl p-4 border border-blue-100">
                        ...
                        <ul className="space-y-2">
                          {insights.recommendations.map((r, i) => (
                            <li key={i} className="text-xs text-gray-700 dark:text-gray-200 flex items-start gap-2">
                          ...
                          <ul className="space-y-2">
                            {insights.atRiskTeams.map((t, i) => (
                              <li key={i} className="text-xs">
                                ...
                                <span className="text-gray-500 dark:text-gray-400 ml-1">— {t.reason}</span>
                              </li>
                            ))}
                          </ul>
                        )}
                        ...
                        <tr key={team.id} className="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                          <td className="py-3 px-2">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-gray-600 dark:text-gray-300 font-semibold">#{i + 1}</span>
                            </div>
                          </td>
                          <td className="py-3 px-2">
                            <span className="font-semibold text-gray-800 dark:text-gray-100">{team.name}</span>
                          ...
                          <td className="py-3 px-2 text-center">
                            <span className="text-gray-700 dark:text-gray-200 font-medium">{team.member_count}</span>
                          </td>
                          <td className="py-3 px-2 text-center">
                            {(() => {
                          ...
                          <td className="py-3 px-2">
                            <div className="flex items-center gap-2 justify-center">
                              ...
                              <span className="text-xs font-medium text-gray-600 dark:text-gray-300 w-8">{progress}%</span>
                            </div>
                          </td>
                          <td className="py-3 px-2 text-center">
                          ...
                          <td className="py-3 px-2 text-center">
                            <span className={`text-xs px-2 py-1 rounded-lg font-bold ${team.grade
                              ...
                              : 'bg-gray-50 dark:bg-slate-800 text-gray-300'
                              }`}>
                              {team.grade || '—'}
                            </span>
                          </td>
                    ...
                    <div key={i} className="flex items-start gap-3 p-3 rounded-xl hover:bg-blue-50/40 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-800 dark:text-gray-100">
                          ...
                          <span className="font-medium text-gray-700 dark:text-gray-200">{activity.team_name}</span>
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">{activity.content}</p>
          ...
          <div className="text-center py-20 bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl border border-white/40 dark:border-slate-700/40 shadow-lg">
            <BarChart3 className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-2">No Data Available</h2>
            <p className="text-gray-500 dark:text-gray-400 mb-4">Import your class data from the Academic Portal to see analytics here.</p>
          </div>
        )}
      </div>
    </AppLayout>
