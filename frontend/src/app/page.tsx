...
export default function Home() {
  ...
  // Function to refetch organizations from backend
  const refetchOrganizations = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const freshOrgs = await response.json()
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!storedToken) {
        ...
        if (scholarSyncToken) {
          // Copy ScholarSync token to SkyFlow
          localStorage.setItem("token", scholarSyncToken)
          storedToken = scholarSyncToken
          console.log("Token shared from ScholarSync to SkyFlow")
        }
      }
      ...
      try {
        ...
        try {
          const prefRes = await fetch(`${API_URL}/api/user/onboarding-preferences`, {
            headers: { 'Authorization': `Bearer ${storedToken}` }
          })
          if (prefRes.ok) {
        ...
        // Get updated orgs from backend
        const meData = await res.json()
        if (meData.organizations) {
          setOrganizations(meData.organizations)
          localStorage.setItem('organizations', JSON.stringify(meData.organizations))
        }
      } catch {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [tasksRes, projectsRes, membersRes] = await Promise.all([
          fetch(`${API_URL}/api/organizations/${selectedOrg.id}/tasks`, { headers: { 'Authorization': `Bearer ${token}` } }),
          fetch(`${API_URL}/api/organizations/${selectedOrg.id}/projects`, { headers: { 'Authorization': `Bearer ${token}` } }),
          fetch(`${API_URL}/api/organizations/${selectedOrg.id}/members`, { headers: { 'Authorization': `Bearer ${token}` } })
        ])
    ...
    const fetchTeamData = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/analytics/overview`, {
          headers: { 'Authorization': `Bearer ${token}`, 'x-organization-id': selectedOrg.id }
        })
  ...
  useEffect(() => {
    const fetchAI = async () => {
      ...
      setAiLoading(true)
      try {
        const res = await fetch(`${API_URL}/api/analytics/ai-insights`, {
          headers: { 'Authorization': `Bearer ${token}`, 'x-organization-id': selectedOrg.id }
        })
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
          ...
          {isTeacher && <QuickAction href="http://localhost:3002" icon={<ExternalLink className="w-4 h-4" />} label="ScholarSync" color="purple" external />}
            ...
            {isTeacher && (
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 backdrop-blur-xl rounded-2xl p-6 border border-green-100 shadow-lg">
                <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3">
                  <FileText className="w-5 h-5 text-green-600" /> ScholarSync
                </h3>
                <p className="text-sm text-gray-600 mb-4">Import team data from Google Sheets to SkyFlow</p>
                <a href="http://localhost:3002" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2.5 px-4 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl text-sm font-medium hover:shadow-lg transition-all">
                  <ExternalLink className="w-4 h-4" /> Open ScholarSync
                </a>
              </div>
            )}
