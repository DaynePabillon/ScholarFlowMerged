        ...
        BEGIN
            ...
            -- If STILL no team_id, try to infer it from assigned_to for regular tasks
            IF v_team_id IS NULL AND TG_TABLE_NAME = 'tasks' AND COALESCE(NEW.assigned_to, OLD.assigned_to) IS NOT NULL THEN
                SELECT team_group_id INTO v_team_id FROM team_group_members WHERE user_id = COALESCE(NEW.assigned_to, OLD.assigned_to) LIMIT 1;
            END IF;
